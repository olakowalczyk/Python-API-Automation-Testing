...
def test_partial_update_booking(token, existing_booking, json_for_patch_booking):
    """
    Checks whether partial update properly updates booking data
    """
    url, _ = existing_booking
    response = HttpManager.patch(url, data=json_for_patch_booking)
